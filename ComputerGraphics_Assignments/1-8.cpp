...
GLvoid drawScene()
{
	...
	for (const auto& position : shapePositions)
	{
		...
		GLfloat p1[3] = { -0.1f * sizeMultipliers[i], -0.1f * sizeMultipliers[i], 1.0f };
		GLfloat p2[3] = { 0.1f * sizeMultipliers[i], -0.1f * sizeMultipliers[i], 1.0f };
		GLfloat p3[3] = { 0.0 * sizeMultipliers[i], 0.1f * sizeMultipliers[i], 1.0f };
		switch (currentShape)
		{
		case 0:
			glBegin(GL_POLYGON);
