...
namespace
{
	std::unique_ptr<Shader> shader, shader_transparent;
...
public:
	...
	{
		for (int i = 0; i < 11; ++i)
			parts[i] = i > 0 && i < 7 ? new ObjectModel(meshCube.get(), shader.get(), texture.get()) : new Object();
...
int main(int argc, char* argv)
{
	...
	shader = std::make_unique<Shader>("yglvert.glsl", "yglfragtexture.glsl");
	shader_transparent = std::make_unique<Shader>("yglvert.glsl", "yglfrag_1-33.glsl");
	...
	texture->LoadFromFile("UV_checker_Map_byValle.jpg");
	...
	for (int i = 0; i < 4; ++i)
	{
		cylinderObstacles[i] = new ObjectModelBlend(meshCylinder.get(), shader_transparent.get(), nullptr, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	for (int i = 0; i < 2; ++i)
	{
		cylinderGate[i] = new ObjectModelBlend(meshCylinder.get(), shader_transparent.get(), nullptr, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	ObjectModel* gateCeil = new ObjectModelBlend(meshCube.get(), shader_transparent.get(), nullptr, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	for (int i = 0; i < TILE_SIZE; ++i)
	{
		...
		{
			floors[i][j] = new ObjectModelBlend(meshCube.get(), shader_transparent.get(), nullptr, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
